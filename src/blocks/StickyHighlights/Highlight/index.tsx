...
export type StickyHighlightsProps = Extract<
  Page['layout'][0],
  { blockType: 'stickyHighlights' }
>

type Fields = Exclude<
  StickyHighlightsProps['stickyHighlightsFields'],
  undefined
>
  ...
  return (
    ...
    >
      <div className={[classes.minHeight, 'grid'].filter(Boolean).join(' ')}>
        <div
          className={[classes.leftContentWrapper, 'cols-4 cols-m-8']
            .filter(Boolean)
            .join(' ')}
        >
      ...
      >
        <Gutter
          className={[classes.codeMediaPosition, 'grid']
            .filter(Boolean)
            .join(' ')}
          ref={nodeRef}
        >
          {type === 'code' && (
            <Fragment>
              <div
                className={[classes.scanlineWrapper, 'start-9 cols-8']
                  .filter(Boolean)
                  .join(' ')}
              >
                <BackgroundScanline
              ...
              >
                ...
                <div className={codeMediaClasses} ref={codeMediaWrapRef}>
                  <div
                    className={classes.codeMediaInner}
                    ref={codeMediaInnerRef}
                  >
