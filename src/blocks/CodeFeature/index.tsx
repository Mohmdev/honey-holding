...
export const CodeFeatureComponent: React.FC<Props> = ({
  ...
  const { alignment, heading, richText, codeTabs, links, settings } =
    codeFeatureFields
    ...
    if (ref) {
      observer = new ResizeObserver((entries) => {
        entries.forEach((entry) => {
          ...
          if (contentBoxSize) {
            const newSize = Array.isArray(contentBoxSize)
              ? contentBoxSize[0]
              : contentBoxSize
  ...
  useEffect(() => {
    const updateBackgroundHeight = () => {
      const newVideoHeight = tabsWrapperRef.current
        ? tabsWrapperRef.current.offsetHeight
        : 0
      ...
      <Gutter>
        {alignment === 'codeContent' ? (
          ...
          >
            ...
            >
              <BackgroundScanline
                className={[classes.scanlineDesktopLeft]
                  .filter(Boolean)
                  .join(' ')}
              ...
              <CrosshairIcon
                className={[classes.crosshairTopLeft].filter(Boolean).join(' ')}
              />

              <CrosshairIcon
                className={[classes.crosshairBottomLeft]
                  .filter(Boolean)
                  .join(' ')}
              />
            </div>
            ...
            <div
              ...
              className={[
                classes.tabsWrapper,
                'cols-10 start-1 cols-m-8 start-m-1'
              ]
                .filter(Boolean)
                .join(' ')}
            >
          ...
          >
            ...
            >
              <BackgroundScanline
                className={[classes.scanlineDesktopRight]
                  .filter(Boolean)
                  .join(' ')}
              ...
              <CrosshairIcon
                className={[classes.crosshairTopRight]
                  .filter(Boolean)
                  .join(' ')}
              />
