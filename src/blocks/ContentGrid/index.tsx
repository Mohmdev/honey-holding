...
export type ContentGridProps = Extract<
  Page['layout'][0],
      ...
      {cells?.map((cell, i) => {
        return (
          <div
            className={[
              classes.cell,
              style === 'sideBySide' ? 'cols-8' : 'cols-4 cols-s-8'
            ]
              .filter(Boolean)
              .join(' ')}
            key={i}
...
export const ContentGrid: React.FC<ContentGridProps> = ({
  ...
  return (
    ...
    >
      ...
      <Gutter
        className={[classes.wrapper, classes[style], 'grid']
          .filter(Boolean)
          .join(' ')}
      >
        <div
        ...
        >
          {content && (
            <RichText
              className={[
                classes.richText,
                style === 'sideBySide' ? 'cols-12' : 'cols-8'
              ]
                .filter(Boolean)
                .join(' ')}
              content={content}
          ...
          {hasLinks && (
            <div
              className={[
                ...
                style === 'sideBySide'
                  ? 'cols-8'
                  : 'cols-4 start-13 cols-l-4 cols-m-8 start-m-1'
              ]
                .filter(Boolean)
                .join(' ')}
            >
