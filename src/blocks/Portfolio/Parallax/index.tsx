...
export const QuoteStickyBlock: React.FC<StickyBlockProps> = (props) => {
  ...
  if (
    ...
    return (
      <div
        className={[classes.stickyBlock, 'grid cols-16 cols-m-8']
          .filter(Boolean)
          .join(' ')}
      >
    ...
    return (
      ...
      >
        ...
        <Gutter className={classes.mainGutter}>
          <Gutter
            className={[classes.scanlineWrapper, 'grid cols-8 start-9']
              .filter(Boolean)
              .join(' ')}
          >
            <BackgroundScanline
              className={[classes.scanline, 'cols-8 start-11']
                .filter(Boolean)
                .join(' ')}
            />
          </Gutter>
          <div
            className={[classes.mainTrack, 'grid'].filter(Boolean).join(' ')}
            ref={containerRef}
          >
          ...
          <div className={classes.navWrapper}>
            ...
            >
              <BackgroundGrid
                zIndex={0}
                className={classes.navBackgroundGrid}
              />

              <Gutter>
                <div
                  className={[classes.navGrid, 'grid']
                    .filter(Boolean)
                    .join(' ')}
                  ref={navGridRef}
                >
                  ...
                  {portfolioParallaxFields?.items.map((item, index) => {
                    return (
                      <div
                        key={index}
                        className={[classes.navItem, `cols-4 cols-m-8`]
                          .filter(Boolean)
                          .join(' ')}
                        ref={(el) => {
