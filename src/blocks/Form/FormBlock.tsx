...
export const FormBlock: React.FC<FormBlockProps> = (props) => {
  const {
    formFields: { richText, form, settings } = {},
    padding,
    hideBackground
  } = props
  ...
  useEffect(() => {
    const updateOuterBackgroundWidth = () => {
      const newOuterBackgroundWidth = sectionRef.current
  ...
  return (
    ...
    >
      ...
      <div
        className={[
          classes.backgroundSectionWrap,
          'cols-12 start-5 cols-m-8 start-m-1'
        ]
          .filter(Boolean)
          .join(' ')}
      >
      ...
      <Gutter className={classes.gutter}>
        <div
          className={[classes.formBlockGrid, 'grid'].filter(Boolean).join(' ')}
        >
          <div
      ...
      <div className={classes.outerBackgroundSectionWrap}>
        <div
          className={classes.outerBackgroundSection}
