...
const LogLine = ({ logLine }: { logLine: LogLine }) => {
  ...
  return (
    <tr
      className={[classes.logLine, classes[`lineType--${lineType}`]].join(' ')}
    >
      {timestamp && (
        <td
          className={classes.logTimestamp}
        >{`[${timestamp.split('.')[0]}]`}</td>
      )}
        ...
        <td>
          {messageChunks.map((chunk, i) => (
            <span
              ...
              className={[
                classes.logText,
                classes[`logTextAppearance--${chunk.appearance}`]
              ].join(' ')}
            >
              {chunk.text}
            </span>
    ...
    const observer = new MutationObserver((mutationsList, observer) => {
      const containerHeightChanged = mutationsList.some((mutation) => {
        return (
          ...
          mutation.target === scrollContent.current
        )
      })
...
export function styleLogLine(logLine: string): LogLine {
  ...
  if (message.startsWith('│')) {
    ...
    messageChunks = [
      {
        appearance:
          message.startsWith('╰') || message.startsWith('╭') ? 'style' : 'text',
        text: message
      }
    ]
  }
