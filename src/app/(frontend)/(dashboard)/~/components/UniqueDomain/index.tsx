...
export const UniqueDomain: React.FC<{
  ...
  const initialSubdomain = useRef<string | undefined>(
    initialValue?.replace('.payloadcms.app', '')
  )

  const [value, setValue] = React.useState<string | undefined>(
    initialSubdomain.current
  )
  ...
  const [validatedDomain, dispatchValidatedDomain] = React.useReducer(
    validatedDomainReducer,
    {
      domain: '',
      isUnique: undefined
    }
  ...
  if (domainIsValid)
    icon = <CheckIcon className={classes.check} size="medium" bold />
