...
import type { Deployment, Plan, Project, Team } from '@dashboard/types'
...
export const useCloudAPI = <R>(args: {
  ...
  const [requestTicker, dispatchRequestTicker] = useReducer(
      ...
      const makeRequest = async (): Promise<void> => {
        try {
          ...
          const req = await fetch(
            `${process.env.NEXT_PUBLIC_CLOUD_CMS_URL}${url}`,
            {
              body,
              credentials: 'include',
              headers: {
...
> = (args) => {
  ...
  const teamsWithoutNone =
    teamsFromArgs?.filter((team) => team !== 'none') || []
  ...
  // const teams =
  //   teamsWithoutNone && teamsWithoutNone?.length > 0
  //     ? teamsWithoutNone
  //     : userTeams
...
export const useGetTeam: UseCloudAPI<Team | null | undefined, string> = (
  teamSlug
) => {
  const response = useCloudAPI<{
    docs: Team[]
    ...
    url: teamSlug
      ? `/api/teams?where[slug][equals]=${teamSlug.toLowerCase()}&limit=1`
      : ''
  })
