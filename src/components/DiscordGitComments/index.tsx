...
import { Messages } from '@root/app/(frontend)/(pages)/community-help/(posts)/discord/[slug]/client_page'
import {
  Answer,
  Comment
...
export const DiscordGitComments: React.FC<CommentProps> = ({
  answer,
  comments,
  platform
}) => {
      ...
      {answer && answer?.body && (
        <li
          className={[
            classes.comment,
            classes.answer,
            answerReplies && classes.hasReplies
          ]
            .filter(Boolean)
            .join(' ')}
        >
          ...
          return (
            <li key={index} className={classes.commentWrap}>
              ...
              >
                ...
                {hasFileAttachments && (
                  <FileAttachments attachments={comment.fileAttachments} />
                )}
