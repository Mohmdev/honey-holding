...
import { GradientBorderIcon } from '@icons/GradientBorderIcon'
...
const CodeBlipButton: React.FC<{
  blip: CodeBlip
  index?: number
  delay?: number
}> = ({ blip, index = 1, delay: delayFromProps = 500 }) => {
  const { isOpen, openModal } = useCodeBlip()

  const style = {
    '--animation-delay': `${delayFromProps * index}ms`
  ...
  return (
    <button
      ...
      aria-pressed={isOpen}
      className={[classes.button, isOpen && classes.hidden]
        .filter(Boolean)
        .join(' ')}
      style={style}
    >
