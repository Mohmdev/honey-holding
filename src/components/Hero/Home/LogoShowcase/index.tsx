...
export const LogoShowcase: React.FC<Props> = ({ logos }) => {
      ...
      setTimeout(() => {
        setLogoPositions((prevPositions) => {
          ...
          return prevPositions.map((pos, idx) =>
            idx === logoIndex
              ? { ...pos, position: newPosition, isVisible: false }
              : pos
          )
        })
  ...
  return (
    <div className={classes.logoShowcase}>
      <div
        className={[classes.horizontalLine, classes.topHorizontalLine]
          .filter(Boolean)
          .join(' ')}
      />
      {[...Array(2)].map((_, idx) => (
      ...
      {[...Array(3)].map((_, idx) => {
        return (
          <div
            key={`v-line-${idx}`}
            className={
              idx === 1 ? classes.verticalLineNoDesktop : classes.verticalLine
            }
            style={{ left: `${(idx + 1) * 25}%` }}
          />
        )
      ...
      <div
        className={[classes.horizontalLine, classes.bottomHorizontalLine]
          .filter(Boolean)
          .join(' ')}
      />
      ...
      <CrosshairIcon
        className={[classes.crosshair, classes.crosshairBottomLeft].join(' ')}
      />
      <CrosshairIcon
        className={[classes.crosshair, classes.crosshairTopRight].join(' ')}
      />
      <CrosshairIcon
        className={[classes.crosshair, classes.crosshairFirstCell].join(' ')}
      />
      <CrosshairIcon
        className={[
          classes.crosshair,
          classes.crosshairSecondRowThirdCell
        ].join(' ')}
      />
    </div>
  )
