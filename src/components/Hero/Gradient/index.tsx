    ...
    >
      {Boolean(fullBackground) && (
        <Media
          className={[
            classes.bgFull,
            enableBreadcrumbsBar ? classes.hasBreadcrumbsEnabled : ''
          ]
            .filter(Boolean)
            .join(' ')}
      ...
      <Gutter>
        <div className={[classes.wrapper, 'grid'].filter(Boolean).join(' ')}>
                  ...
                  links.map(({ link }, i) => {
                    return (
                      <CMSLink
                        ...
                        className={[classes.link, 'cols-12 start-1']
                          .filter(Boolean)
                          .join(' ')}
                      />
                    )
          ...
          {!Boolean(fullBackground) && (
            <Media
              className={[classes.bgSquare, 'cols-8 start-9 start-m-1']
                .filter(Boolean)
                .join(' ')}
          ...
          >
            {images && Array.isArray(images) && (
              <MediaParallax media={images} priority />
            )}
          </div>
        </div>
