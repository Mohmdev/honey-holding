      ...
      <div className={classes.livestreamHero}>
        ...
        <div className={classes.bg2Wrapper}>
          <Gutter className={classes.bgGutter}>
            <div
              className={[classes.bg2Grid, 'grid'].filter(Boolean).join(' ')}
            >
              <div
                className={[
                  classes.bg2Cell,
            ...
            >
              ...
              {guests &&
                ...
                guests.map(({ name, link, image }, i) => {
                  return (
                    <a
                      className={classes.guestWrap}
                      key={i}
                      href={link || '/'}
                      target="_blank"
                    >
                      {image && typeof image !== 'string' && (
                        <Media src={`${getClientSideURL()}${image.url}`} />
                      )}
                      {name && name}
                    </a>
                  )
            ...
            {!isLive && (
              ...
              >
                <div className={classes.linksWrap}>
                  {date && (
                    <label>
                      Starting {formatDate({ date, format: 'dateAndTime' })}
                    </label>
                  )}
                  &nbsp;
                  {Array.isArray(links) &&
                    links.map(({ link }, i) => {
                      ...
                      return (
                        <Button
                          ...
                          className={[
                            classes.link,
                            appearance && classes[`link--${appearance}`]
                          ]
                            .filter(Boolean)
                            .join(' ')}
                          el="a"
